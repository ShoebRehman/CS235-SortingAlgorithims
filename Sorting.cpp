...
Name: Shoeb Rehman
Date: March 5, 2014
About: This driver program does four specific sorts: Insertion Sort, Bubble Sort, Merge Sort, and Quick Sort.
It then outputs the time (in seconds) it takes for the sort to complete. It also allows the user to input how large they want the array to be.
*/
...
void merge(int* copiedArray, int low, int mid, int high);
...
int main(){
	...
	char ans = 'y';
	do{
		int sizeArray = selectSize(); //sets the size of the array to what the user specified
		int* array = new int[sizeArray];
		int* copiedArray = new int[sizeArray];
		fillArray(array, sizeArray);
		for (int j = 1; j < 5; j++){
			if (j == 1){
				copyArray(array, copiedArray, sizeArray);
				insertionSort(copiedArray, sizeArray);
			}

			if (j == 2){
				copyArray(array, copiedArray, sizeArray);
				bubbleSort(copiedArray, sizeArray);
			}

			if (j == 3){
				copyArray(array, copiedArray, sizeArray);
				mergeSort(copiedArray, sizeArray);
			}

			if (j == 4){
				copyArray(array, copiedArray, sizeArray);
				quickSort(copiedArray, sizeArray);
			}
		}

		cout << "Would you like to try another array size? y/n" << endl;
		cin >> ans;
		delete[] array; //deletes dynamically allocated array
		delete[] copiedArray;
